...
## [2026-06-08] rendimiento | RAM residual tras analizar: high-water-mark del allocator, NO fuga
El `benchmark_live_history.jsonl` de la Capa 2 había registrado (2026-06-05) un RSS de **5.346 MB en estado `tabla-llena`** (idle, CPU 3%) que **no bajaba** de los ~223 MB de `reposo`, y un pico de 6.760 MB en `analizando`. El pico alto es esperable: la GUI analiza con `ThreadPoolExecutor(max_workers=min(4, cpu))` → CPU 423% ≈ 4 cores, ~1 GB de working set de STFT por archivo en vuelo × 4. Lo dudoso era el residual idle. **Diagnóstico de dos tandas** (script puntual, RSS *instantáneo* con psutil — `ru_maxrss` no sirve porque es monotónico): corriendo el batch secuencial dos veces en el mismo proceso, la línea base ociosa fue **3.879,8 MB tras la tanda 1 → 3.775,6 MB tras la tanda 2 (−2,7 %)**. No crece → **high-water-mark del allocator**, no fuga: la 2ª tanda reutiliza la RAM de la 1ª. La evicción de `frequency_analysis=None` (ver [memory-management](../../.claude/rules/memory-management.md)) sí ocurre; lo que queda es que numpy/scipy liberan los arrays pero el allocator **no devuelve las páginas al SO** (macOS no tiene `malloc_trim`), así que la app se queda en varios GB de RSS hasta cerrarse. Benigno en términos de fuga, pero es RSS real que el desktop retiene; el techo lo fija el pico de STFT por archivo (×concurrencia). Acción tomada: `tests/benchmark_live.py` ahora **avisa solo** cuando un estado idle (`tabla-llena`/`watcher`) supera 2× el RSS de `reposo` (`check_idle_rss`), para que el instrumento grite y no dependa de leer bien la cifra. Pendiente opcional: acotar el pico de STFT en pistas largas si se quiere bajar el techo de RSS.
